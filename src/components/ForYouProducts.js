...
function ForYouProducts() {
    ...
    React.useEffect(() => {
        const fetchRecommendedProducts = async () => {
            ...
            try {
                ...
                const { data } = await axios.get('http://127.0.0.1:8000/like-recommendations/', {
                    headers: {
    ...
    if (!recommendedProducts || recommendedProducts.length === 0) {
        return <Text>No recommended products available.</Text>;
    }

    return (
        <Box p={4} mt={8}>
            <Text fontSize="2xl" mb={4} fontWeight="bold">For You</Text>
